package com.example.trellassignment.util
...
object StorageUtility {
    ...
    fun getStorageDirectories(context: Context): Array<String?> {
        // Final set of paths
        val availableDirectoriesSet: MutableCollection<String?> =
            HashSet()
    ...
    private fun getExternalStorage(context: Context): Set<String> {
        val availableDirectoriesSet: MutableSet<String> =
            HashSet()
    ...
    private val allSecondaryStorages: Array<String?>
        private get() = if (!TextUtils.isEmpty(SECONDARY_STORAGES)) {
            // All Secondary SD-CARDs split into array
            SECONDARY_STORAGES.split(File.pathSeparator)
                .toTypedArray()
        } else arrayOfNulls(0)
